...
app.post('/generate-clients', async (req, res) => {
    try {
        const count = req.body.count || 5;
        const created = [];
        for (let i = 0; i < count; i++) {
            const email = `test${Date.now()}_${i}@test.com`;
            const u = await admin.auth().createUser({ email, password: '123456' });
            const score = Math.floor(Math.random() * 1000);
            await db.collection('users').doc(u.uid).set({ email, score });
            created.push({ uid: u.uid, email, score });
        }
        res.json({ created });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/top5', async (req, res) => {
    try {
        const snap = await db.collection('users').orderBy('score', 'desc').limit(5).get();
        const top = [];
        snap.forEach(d => top.push({ uid: d.id, email: d.data().email, score: d.data().score }));
        res.json({ top });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
